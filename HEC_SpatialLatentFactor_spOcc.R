...
rem = which(is.na(occ$deforest_1km) | is.na(occ$deforest_2km) | is.na(occ$deforest_5km) | occ$prim_2nd=="not_noted")
occ = occ[-c(rem), ]
...
### Change Deforestation from Ha to Proportion of buffer zone
### 100Ha = 1sq km
### 1km buffer = 3.14sqkm = 314Ha
### 2km buffer = 12.57sqkm = 1257Ha
### 5km buffer = 78.54sqkm = 7854Ha
occ.covs$deforest_1km = occ.covs$deforest_1km/314
occ.covs$deforest_2km = occ.covs$deforest_2km/1257
occ.covs$deforest_5km = occ.covs$deforest_5km/7854
...
buffer = c(occ.covs[,3:5])
bufName = c("5km", "2km", "1Km")
for (jj in 1:length(buffer)) {
  #  jj=1
  ...
  out.sfMsPGOcc <- sfMsPGOcc(occ.formula = ~ occ.covs[,2+jj] + as.factor(habitat) + as.factor(prim_2nd),
  ...
  # plot(out.sfMsPGOcc$beta.samples[, 1:4], density = FALSE) # Plot Convergence of 1st 6 species
  filename = paste0("/storage/hpc/44/slatera6/SpLF3_DetOcc_",reps[j],"_buffer",bufName[jj], ".Rdata")
  save(out.sfMsPGOcc,file = filename)
  rm(out.sfMsPGOcc); gc()
  # } # End Replicates loop 
} # End Buffer Size loop
