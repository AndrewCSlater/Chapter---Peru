...
# colnames(ssPnt) # Stations
# rownames(ssPnt) # Species
# 
# colnames(ssPnt[1,,]) # Replicates
# rownames(ssPnt[1,,]) # Stations
# 
# colnames(ssPnt[,1,]) # Replicates
# rownames(ssPnt[,1,]) # Species
# 
# colnames(ssPnt[,,1]) # Stations
# rownames(ssPnt[,,1]) # Species

### Replicates
cc=c()
for (i in 1:nrow(ssPnt[1,,])) {    # For each row (Station)
print(length(which(!is.na(ssPnt)[1,i,])))
cc = append(cc, length(which(!is.na(ssPnt)[1,i,])))  ## get the number of Non-NA (replicates that were conducted) 
}
hist(cc, breaks = 35)
summary(cc)
quantile(cc, probs = c(0.9, 0.95, 0.975))

length((ssPnt)[1,,]) # 22645 
length(which(is.na(ssPnt)[1,,])) # 19348
length(which(!is.na(ssPnt)[1,,])) # 3297

## Sites X Replicates = 22,645
## But in only 3,297 of those (14.6%) is a survey conducted

## Median number of replicates is 4, and Mean is 5.1 
### 75% of sites have 7 or fewer
### 90% have 10 or fewer
### 95% have 14 or fewer 
### 97.5% have 18 or fewer

## I am keeping only the first 14 replicates, which covers 95% of the data
ssPnt.reduced = ssPnt[,,1:14]

#######
## Remove those species occurring in only 1 station
rem = c()
for (i in 1:451) {
  # i = 100
  df = ssPnt.reduced[i,,]
...
ssPnt_MinSp = ssPnt.reduced[-c(rem),,]
ssPnt = ssPnt_MinSp
sp.names = rownames(ssPnt)    # 275 of 451 species remaining
...
colnames(ssPnt)[!colnames(ssPnt) %in% occ$site_station]

rem = which(!colnames(ssPnt) %in% occ$site_station)
colnames(ssPnt)[rem] 
...
occ.covs = occ[,c(14:15)]

# miss = apply(X = is.na(sat), MARGIN = 2, FUN = sum) ## Number of NA per column
# rem = which(miss>1)  ## Those columns with more than 1 NA
# if (length(rem)>0) { sat = sat[,-rem]}
# 
# # there are a couple of NA values which we can't have, so I set them here to the mean of the column
# for(i in 1:ncol(sat)){
#   sat[is.na(sat[,i]), i] <- mean(sat[,i], na.rm = TRUE)
# }
# occ.covs = cbind(grad$Gradient, sat)


## Y Counts
#  As Sites have been removed, there may now be species with Zero Counts that must be removed
rem = c()
for (i in 1:length(sp.names)) {   # For each species
# i = 100
df = ssPnt1[i,,]
site = df
site = rowSums(df, na.rm = T)
...
# ssPnt1 = ssPnt1[-c(rem),,]
sp.names = rownames(ssPnt1) # 275 Species
...
o = order(apply(ssPnt1, 1, sum, na.rm = TRUE), decreasing = T)
o = o[c(1, length(o), 3:length(o)-1)]
sp.ordered = sp.names[o]
y.new <- ssPnt1[sp.ordered, , ]
...
det.covs = list(method = method, am.pm = am.pm, effort = effort, year = year)
...
# keep = which(coords$site_station %in% colnames(ssPnt1[,,1]))           # Find which Station Names occur in the Count Array
...
# or = match(x = colnames(ssPnt1[,,1]), table = coords$site_station)     # Get the order in which the Count names match the coord names 
...
str(data)
is.na(det.covs)
## We will model occurrence for all species as a function of linear and quadratic terms for elevation, and detection as a function of linear and quadratic day of survey as well as the time of day during which the survey was conducted.

# ## For multi-species models, the multi-species detection-nondetection data y is now a three-dimensional array with dimensions corresponding to species, sites, and replicates.
# det.ms.formula <- ~ scale(surv.time)

# results = matrix(data = NA, nrow = ncol(occ.covs), ncol = 2)
# colnames(results) = c("Variable", "WAIC")
# results[,1] = colnames(occ.covs)
