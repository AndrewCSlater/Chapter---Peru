...
ssPnt[ssPnt>0]=1
# table(ssPnt) # Quick summary 10622 observations - 1597644 Non-Observations
# str(ssPnt)
...
rem = which(!occ$site_station %in% colnames(ssPnt.reduced)) # Remove Occurrence vars where there are no stations in counts
if (length(rem)==0) {occ = occ
} else {occ = occ[-rem,]}
rem = which(!colnames(ssPnt.reduced) %in% occ$site_station) # Remove Counts where there are no stations in Occ vars
if (length(rem)==0) {ssPnt1 = ssPnt.reduced
} else {ssPnt1 = ssPnt.reduced[,-rem,]}
...
colSums(is.na(occ))
occ.covs = occ[,c(14:15)]  ## Habitat-Floodplain/TerraFirma - Primary/Secondary
...
for (i in 1:451) {
  # i = 100
  df = ssPnt1[i,,]     # Create a single df relating to the current bird species (i)
  site = df
  site = rowSums(df, na.rm = T)  # Get row sums - ie how many times was it seen in each station
  site[site>0]=1                 # Turn the number of times per station into a single presence/absence 1/0 per station
  if (sum(site)<2) {             # If the sum of all station is less than 2, the species only occurred in 1 (or 0) station  
#if (sum(colSums(df,na.rm = T))<2) {
    rem = append(rem, i)         # If the species only occurred in 1 or 0 stations, add the bird to the remove list
  }
  # print(sum(df, na.rm = T))
}
ssPnt_MinSp = ssPnt1[-c(rem),,]  # Remove those species from the full data set
ssPnt = ssPnt_MinSp
sp.names = rownames(ssPnt)    # 268 of 451 species remaining
...
if (length(rem==0)) {ssPnt = ssPnt[-c(rem),,]}
sp.names = rownames(ssPnt) # 268 Species
...
o = order(apply(ssPnt, 1, sum, na.rm = TRUE), decreasing = T)
o = o[c(1, length(o), 3:length(o)-1)]
sp.ordered = sp.names[o]
y.new <- ssPnt[sp.ordered, , ]
...
XY = coords[,1:2]                                                ####### Coordinates Must Be PROJECTED !!! ###########
...
# 
# save(out.intercept, file = "model-1-INTERCEPT_5000its_2000brn_10thn_4ch_1200smpl.Rdata")
...
## specify the number of threads to use (n.omp.threads), the number of MCMC samples (n.samples), the amount of samples to discard as burn-in (n.burn), the thinning rate (n.thin), and arguments to control the display of sampler progress (verbose, n.report).
form = c(~ method, ~ effort, ~ year, ~ am.pm)
nn = c("method", "effort", "year", "am.pm")
for (i in 1:length(form)) {
  out <- msPGOcc(occ.formula = ~ 1, 
                    ...
                    n.chains = 4)# ,
# k.fold = 4, k.fold.threads = 4, k.fold.only = F)
assign(x = paste0("out_",nn[[i]]), value = out.detAMPM)
# summary(out.detAMPM)
# a=out.detAMPM$alpha.samples
# str(a)
# mean(a[,1])
# a[3,]
# summary(a[,3])
filename = paste0("models-1-Detection_",nn[[i]],".Rdata")
