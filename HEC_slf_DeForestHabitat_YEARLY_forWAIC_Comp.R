...
# occ.covs = full_join(frst4, ref3, by="site_station")
...
data = list(y = y.new, occ.covs = frst4, det.covs = det.covs, coords = XY)

### 6 - Spatial Latent factor Model
out.SpLaFa_habPRIM <- sfMsPGOcc(occ.formula = ~ Habitat + Forest + scale(deforest_1km) + scale(deforest_2km) + scale(deforest_5km),
                                # PC1 + PC2 + PC3 + PC4 + PC5,
                                # 
                                ...
                                n.thin = 50,
                                n.chains = 1)
save(out.SpLaFa_habPRIM, file = "1.B-WAIC_Comparison_Min11sp_DeForestHabitat_50000_25000_50_1.Rdata")
