...
pca_occ = as.data.frame(pca_ref2$x)
write.csv(pca_occ, "/storage/hpc/44/slatera6/SpLF3_DetOcc_PCA1_12reps_buffer500m_PCA_Components.csv", row.names = T)

######################################################
### Plot PCAs grouped by Env Vars
df2 = ref2

df2$site_station = gsub(pattern = "_20.*$",replacement = "",x = df2$site_station)
...
df2 = df2[df2$site_station %in% env$site_station, ]
df3 = left_join(df2, env, by = "site_station")
df3 = df3[df3$prim_2nd!="not_noted",]
...
# df2 = select(df2, -"site_station", -"year")
#calculate principal components
...
occ = as.matrix(pca_occ[,1])
colnames(occ) = "PC1"

#####
##### - PUT ALL VARIABLES IN A LIST - #####
data = list(y = y.new, occ.covs = occ, det.covs = det.covs, coords = XY)
# str(data)
# is.na(det.covs)
# range(is.na(pca_occ[,1]))
...
out.sfMsPGOcc <- sfMsPGOcc(occ.formula = ~ PC1,
...
filename = paste0("/storage/hpc/44/slatera6/SpLF3_DetOcc_PCA1_12reps_buffer500m_MODEL.Rdata")
...
filename = paste0("/storage/hpc/44/slatera6/SpLF3_DetOcc_PCA1_12reps_buffer500m_FITTED_SAMPLES.Rdata")
...
write.csv(auc.vals, "/storage/hpc/44/slatera6/SpLF3_DetOcc_PCA1_12reps_buffer500m_AUC.csv", row.names = F)

load("/storage/hpc/44/slatera6/SpLF3_DetOcc_PCA1_12reps_buffer500m_MODEL.Rdata")
ppc = ppcOcc(out.sfMsPGOcc, fit.stat = 'freeman-tukey', group = 1) # Group 1 = By Station
save(ppc, file = "/storage/hpc/44/slatera6/SpLF3_DetOcc_PCA1_12reps_buffer500m_Bayesian_P.Rdata")
