...
spCnt =c()
for (j in 1:nrow(ssPnt.14)) {
x = colSums(ssPnt.14[j,,], na.rm = T)
spCnt = cbind(spCnt,x)
  }
...
for (j in 1:length(surveys)) {
  ...
  out.det <- msPGOcc(occ.formula = ~ deforest_5km + deforest_2km + deforest_1km + habitat + as.factor(prim_2nd), 
                     det.formula = ~ 1,
                     data = data, 
                     # inits = ms.inits, 
                     n.samples = 10000, 
  ...
  filename = paste0("/storage/hpc/44/slatera6/OCCURRENCE_ONLY_",reps[j],".Rdata")
